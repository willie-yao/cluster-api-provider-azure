...
func registerWebhooks(mgr manager.Manager) {
	...
	if feature.Gates.Enabled(capifeature.MachinePool) {
		if err := infrav1expbeta1.SetupAzureMachinePoolWebhookWithManager(mgr); err != nil {
			setupLog.Error(err, "unable to create webhook", "webhook", "AzureMachinePool")
			os.Exit(1)
		}

		if err := (&infrav1expbeta1.AzureMachinePoolMachine{}).SetupWebhookWithManager(mgr); err != nil {
