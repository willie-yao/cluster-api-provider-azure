...
| Resource | Purpose |
| --- | --- |
| **AKS management cluster** (`Standard_B2s` × 2) | Hosts CAPI Operator + CAPZ. ~$70/mo if left running. |
...
1. Create a resource group:
   ...
   # replicated. As of 2026-05, this is eastus, eastus2, and westus2.
   # westus3 / westus / centralus do NOT have the gallery; deployments
   # there fail with GalleryImageNotFound.
   az group create -n capz-preview-rg -l eastus
   ```
...
4. Pull the workload kubeconfig out of Key Vault:
   ...
   az keyvault secret show \
     --vault-name $(az deployment group show -g capz-preview-rg \
                      -n main --query properties.outputs.kubeconfigVault.value -o tsv) \
     --name workload-kubeconfig \
     --query value -o tsv > workload.kubeconfig
...
| `controlPlaneMachineCount` | `1` | Replica count for the workload cluster control plane. |
...
| `previewRepoUrl` / `previewRepoRef` | upstream CAPZ / `main` | The deploymentScript clones this to find `kustomize` overlays and feature recipes. Pin to a release tag for reproducible deploys. |
...
Rough monthly cost if left running 24/7 (East US prices):

| Resource | ~ $/mo |
| --- | --- |
| AKS (2 × `Standard_B2s`, free control plane tier) | $30 |
| Workload control plane (1 × `Standard_D2s_v3`) | $70 |
| Workload workers (2 × `Standard_D2s_v3`) | $140 |
| LBs, disks, public IP, Key Vault | < $15 |
| **Total** | **~$255** |

Tear it down when you're done — `az group delete -n <rg>` removes
everything.
...
2. **`bootstrap.sh`** clones this repo, runs `az aks get-credentials`,
   ...
   for Workload Identity, then `kustomize build` + `envsubst` +
   `kubectl apply` the workload cluster spec.
...
2. Re-deploy with `-p ciVersion=<ver>` — the existing AKS + UAMI +
   Key Vault stay; the deploymentScript re-runs and `kubectl apply`s
   an updated KCP, which rolls the control plane and node pools.

If a new alpha breaks bringup with the blanket `AllAlpha=true`, fall back
to listing only the gates you care about:

```bash
az deployment group create -g capz-preview-rg -f main.bicep \
  -p ciVersion=<ver> \
