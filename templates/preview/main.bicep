...
@description('Suffix appended to resource names to keep them globally unique within the RG.')
param nameSuffix string = uniqueString(resourceGroup().id, deployment().name)

@description('Azure region to deploy into. Must support deploymentScripts + AKS + Key Vault.')
...
@description('Replica count for the workload cluster control plane. Internal mode requires 3 for apiserver-ILB hairpin routing; defaults to 1 in public mode.')
@allowed([
  1
  3
  5
])
param controlPlaneMachineCount int = (networkingMode == 'internal') ? 3 : 1
...
@description('Repository hosting the templates/preview tree (kustomize + features). The deploymentScript clones this at the ref below.')
param previewRepoUrl string = 'https://github.com/kubernetes-sigs/cluster-api-provider-azure.git'

@description('Git ref (branch or tag) to clone from previewRepoUrl. Pin to a release tag for reproducible deploys.')
param previewRepoRef string = 'main'

@description('Workload cluster name (also used as the Cluster CR name).')
param workloadClusterName string = 'capz-preview'

@description('Marker that forces the deployment script to re-run. Defaults to the deployment time so a re-deploy always re-bootstraps.')
...
resource bootstrap 'Microsoft.Resources/deploymentScripts@2023-08-01' = {
  ...
  properties: {
    ...
    environmentVariables: [
      ...
      { name: 'CONTROL_PLANE_MACHINE_COUNT',    value: string(controlPlaneMachineCount) }
