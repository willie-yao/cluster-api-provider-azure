...
func TestReconcileVM(t *testing.T) {
	testcases := []struct {
		...
		expect        func(s *mock_virtualmachines.MockVMScopeMockRecorder, mnic *mock_async.MockGetterMockRecorder, mpip *mock_async.MockGetterMockRecorder, r *mock_aso.MockReconcilerMockRecorder[*asocomputev1.VirtualMachine])
	}{
		{
	...
	for _, tc := range testcases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			asoMock := mock_aso.NewMockReconciler[*asocomputev1.VirtualMachine](mockCtrl)

			tc.expect(scopeMock.EXPECT(), interfaceMock.EXPECT(), publicIPMock.EXPECT(), asoMock.EXPECT())
...
func TestDeleteVM(t *testing.T) {
	testcases := []struct {
		name          string
		expectedError string
		expect        func(s *mock_virtualmachines.MockVMScopeMockRecorder, r *mock_aso.MockReconcilerMockRecorder[*asocomputev1.VirtualMachine])
	}{
		{
			name:          "noop if no vm spec is found",
			expectedError: "",
