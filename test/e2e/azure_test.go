...
var _ = Describe("Workload cluster creation", func() {
	...
	Context("Creating a self-managed VM based cluster using API Server ILB feature gate and custom images [OPTIONAL][API-Server-ILB]", func() {
		It("with three controlplane node and three worker nodes", func() {
			clusterName = getClusterName(clusterNamePrefix, "apiserver-ilb")
			...
			clusterctl.ApplyClusterTemplateAndWait(ctx, createApplyClusterTemplateInput(
				specName,
				withFlavor("apiserver-ilb-custom-images"),
				withNamespace(namespace.Name),
				withClusterName(clusterName),
				withControlPlaneMachineCount(3),
				withWorkerMachineCount(2),
