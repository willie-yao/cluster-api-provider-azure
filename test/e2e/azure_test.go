...
var _ = Describe("Workload cluster creation", func() {
	...
	Context("Creating a willie test cluster [REQUIRED]", func() {
		It("With 1 control-plane nodes and 1 Linux node", func() {
			clusterName = getClusterName(clusterNamePrefix, "willie")
			...
			clusterctl.ApplyClusterTemplateAndWait(ctx, clusterctl.ApplyClusterTemplateAndWaitInput{
				...
				WaitForClusterIntervals:      e2eConfig.GetIntervals(specName, "wait-cluster"),
				WaitForControlPlaneIntervals: e2eConfig.GetIntervals(specName, "wait-control-plane-willie"),
				WaitForMachineDeployments:    e2eConfig.GetIntervals(specName, "wait-worker-nodes"),
				ControlPlaneWaiters: clusterctl.ControlPlaneWaiters{
					WaitForControlPlaneInitialized: EnsureControlPlaneInitialized,
				},
